...
#endif
            if(ReferenceEquals(body, null)) throw new ArgumentNullException(nameof(body));
            var requestInfo = new RequestInformation(Method.DELETE, "{+baseurl}/devices/{deviceId}/incomings{?confirmed*}", PathParameters);
...
#nullable restore
        ...
        {
#endif
            var requestInfo = new RequestInformation(Method.GET, "{+baseurl}/devices/{deviceId}/incomings{?pg%5Blimit%5D*,pg%5Boffset%5D*,pg%5BsortBy%5D*,pg%5BsortDir%5D*,scope*}", PathParameters);
            requestInfo.Configure(requestConfiguration);
            requestInfo.Headers.TryAdd("Accept", "application/json");
            return requestInfo;
        }
